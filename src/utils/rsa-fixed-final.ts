...
export class RSAEncryptFixed {
  ...
  static setPublicKey(publicKey: string): void {
    ...
    try {
      ...
      if (!key) {
        throw new Error("公钥设置失败：getKey()返回null");
      }
      ...
      if (!testResult) {
        console.warn("[RSA] 警告：测试加密返回null，但公钥已设置");
        // 不抛出错误，继续尝试
      } else {
      ...
      console.error("[RSA] 公钥设置失败:", error.message);

      // 尝试其他格式
      console.log("[RSA] 尝试其他格式...");

      try {
        // 尝试原始Base64格式
        this.encryptor = new JSEncrypt();
        this.encryptor.setPublicKey(publicKey);
        console.log("[RSA] 使用原始Base64格式成功");
        this.isInitialized = true;
      } catch (e2: any) {
        console.error("[RSA] 原始Base64格式也失败:", e2.message);
        throw new Error(`RSA公钥设置失败: ${error.message}`);
      }
    }
  }
